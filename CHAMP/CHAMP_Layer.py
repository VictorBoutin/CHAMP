        ...
        for i_epoch in range(self.nb_epoch):
            ...
            for idx_batch, batch  in enumerate(data_set[0]):
                ...
                if self.mode == 'Hebbian':
                    dictionary =  learn2(self.code, self.dictionary, self.residual_image,self.eta,self.mask)
                elif self.mode == 'SVD':
...
def EnergicConvMP_np(image_input, dictionary, l0_sparseness=2,
    ...
    for i_m in range(nb_image):
        ...
        while c_ind > 0.001:
            ...
            if alpha is not None :
                c_ind = alpha*c_ind
            energy_before = np.sum(np.linalg.norm(I_conv_padded[i_m, :, indice[2]:indice[2] + X_conv_size[0], indice[3]:indice[3] + X_conv_size[1]],axis=(1,2)))
            I_conv_padded[i_m, :, indice[2]:indice[2] + X_conv_size[0], indice[3]:indice[3] + X_conv_size[1]]+= -c_ind * X_conv[indice[1], :, :, :]
            energy_after = np.sum(np.linalg.norm(I_conv_padded[i_m, :, indice[2]:indice[2] + X_conv_size[0], indice[3]:indice[3] + X_conv_size[1]],axis=(1,2)))
            alpha_energy = energy_after/energy_before
            I_conv_padded[i_m, :, indice[2]:indice[2] + X_conv_size[0], indice[3]:indice[3] + X_conv_size[1]]+= c_ind * alpha_energy * X_conv[indice[1], :, :, :]
...
def learn2(code,dictionary,residual,eta,mask):
    ...
    for idx_dico in range(nb_dico):
        ...
        if len(loc_image) != 0:
            ...
            for idx in range(len(loc_image)) :
                patch = residual[loc_image[idx],:,loc_line[idx]:loc_line[idx]+dico_size[1],loc_col[idx]:loc_col[idx]+dico_size[2]]
                #if do_mask == False :
                #    all_patches[idx,:,:,:] = act_c[idx]*patch
                #else :
                all_patches[idx,:,:,:] = act_c[idx]*(patch*mask[idx_dico,:,:,:]-act_c[idx]*dictionary[idx_dico,:,:,:].numpy()*(1-mask[idx_dico,:,:,:]))
    ...
    for idx_dico in range(nb_dico):
        ...
        if len(loc_image)>1:
            ...
            for idx in range(len(loc_image)) :
                residual[loc_image[idx],:,loc_line[idx]:loc_line[idx]+dico_size[-1],loc_col[idx]:loc_col[idx]+dico_size[-2]] \
                -= code[loc_image[idx],idx_dico,loc_line[idx],loc_col[idx]] * dictionary[idx_dico,:,:,:]
            dictionary[idx_dico,:,:,:] = torch.FloatTensor(u[:,0].reshape(dico_size[0],dico_size[1],dico_size[2]))
    return dictionary
