...
class CHAMP_LayerV2():
    ...
    def TrainLayer(self, data_set, eta=0.05, nb_epoch=2, eta_homeo=0.01, nb_record=5):
        ...
        for i_epoch in range(self.nb_epoch):
            for i,image in enumerate(data_set[0]) :
                code, res, nb_activation, all_patches, all_patches_idx = self.CodingCHAMP(image, self.dictionary, \
    ...
    def CodingCHAMP(self, image_input, dictionary, l0_sparseness=2, \
        ...
        all_patches = torch.zeros(l0_sparseness* nb_image,1,dictionary.size()[2],dictionary.size()[3])
        all_patches_idx = torch.zeros( l0_sparseness* nb_image)
        ...
        for i_m in range(nb_image):
            ...
            for i_l0 in range(l0_sparseness):
                ...
                if train == True:
                    ...
                    patch = residual_image[i_m,0,indice[2]:indice[2]+padding+1,indice[3]:indice[3]+padding+1]
                    #all_patches[idx,0,:,:] = residual_image[i_m,0,indice[2]:indice[2]+padding+1,indice[3]:indice[3]+padding+1]
                    all_patches[idx,0,:,:] = c_ind*dictionary[indice[1],0,:,:].data
                    all_patches_idx[idx] = int(indice[1])
        ...
        return code, residual_image, nb_activation, all_patches, all_patches_idx, reconstructed_image
