    ...
    def TrainNetwork(self, training_set, testing_set, eta=0.05, eta_homeo=0.01,
        ...
        for idx, each_Layer in enumerate(self.Layers):
            if each_Layer.type == 'Unsupervised':
                dico = each_Layer.TrainLayer(
                    training_set, eta=eta, eta_homeo=eta_homeo, nb_epoch=epoch_unsupervised)
                training_set = (training_set[0].view(
                    60000//batch_size_cl, batch_size_cl, 1, 28, 28), training_set[1].view(60000//batch_size_cl, batch_size_cl))
